...
function update() {
  ...
  if (score >= 200) { gameRunning = false; setTimeout(()=>show('bintang'), 800); }

  requestAnimationFrame(update);
}

function jump() { if (dino.grounded) { dino.grounded = false; dino.dy = dino.jump; } }
function gameOver() {
  gameRunning = false;
...
function startGame() {
  ...
  dino.y = 200; gameRunning = true; update();
}
